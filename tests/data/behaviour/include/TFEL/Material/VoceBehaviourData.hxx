/*!
* \file   TFEL/Material/VoceBehaviourData.hxx
* \brief  this file implements the VoceBehaviourData class.
*         File generated by tfel version 4.0.0-dev
* \author Ds
* \date   02 / 04 / 2021
 */

#ifndef LIB_TFELMATERIAL_VOCE_BEHAVIOUR_DATA_HXX
#define LIB_TFELMATERIAL_VOCE_BEHAVIOUR_DATA_HXX
...
class VoceBehaviourData;

//! \brief forward declaration
template<ModellingHypothesis::Hypothesis hypothesis, typename NumericType,bool use_qt>
class VoceIntegrationData;

//! \brief forward declaration
template<ModellingHypothesis::Hypothesis hypothesis,typename NumericType>
std::ostream&
 operator <<(std::ostream&,const VoceBehaviourData<hypothesis, NumericType,false>&);

//! \brief forward declaration
template<typename NumericType>
std::ostream&
 operator <<(std::ostream&,const VoceBehaviourData<ModellingHypothesis::AXISYMMETRICALGENERALISEDPLANESTRESS, NumericType, false>&);

//! \brief forward declaration
template<typename NumericType>
std::ostream&
 operator <<(std::ostream&,const VoceBehaviourData<ModellingHypothesis::PLANESTRESS, NumericType, false>&);

template<ModellingHypothesis::Hypothesis hypothesis, typename NumericType>
class VoceBehaviourData<hypothesis, NumericType,false>
{
...
friend std::ostream& operator<< <>(std::ostream&,const VoceBehaviourData&);

/* integration data is declared friend to access   driving variables at the beginning of the time step */
friend class VoceIntegrationData<hypothesis, NumericType, false>;
...
*/
VoceBehaviourData(const VoceBehaviourData& src)
: eto(src.eto),
sig(src.sig),
eel(src.eel),
p(src.p),
 ...
 */
VoceBehaviourData(const mfront::gb::BehaviourData& mgb_d)
...
*/
VoceBehaviourData&
operator=(const VoceBehaviourData& src){
this->eto = src.eto;
this->sig = src.sig;
this->eel = src.eel;
this->p = src.p;
...
}; // end of VoceBehaviourDataclass

template<ModellingHypothesis::Hypothesis hypothesis, typename NumericType>
std::ostream&
operator <<(std::ostream& os,const VoceBehaviourData<hypothesis, NumericType, false>& b)
{
os << "εᵗᵒ : " << b.eto << '\n';
os << "σ : " << b.sig << '\n';
os << "εᵉˡ : " << b.eel << '\n';
...
template<typename NumericType>
class VoceBehaviourData<ModellingHypothesis::AXISYMMETRICALGENERALISEDPLANESTRESS, NumericType, false>
{
...
: eel(&mgb_d.s0.internal_state_variables[0]),
p(mgb_d.s0.internal_state_variables[StensorSize]),
etozz(mgb_d.s0.internal_state_variables[StensorSize+1]),
T(mgb_d.s0.external_state_variables[0]),
...
tfel::math::map<StrainStensor>(mbg_s1.internal_state_variables) = this->eel;
mbg_s1.internal_state_variables[StensorSize] = this->p;
mbg_s1.internal_state_variables[StensorSize+1] = this->etozz;
} // end of exportStateData
...
template<typename NumericType>
std::ostream&
operator <<(std::ostream& os,const VoceBehaviourData<ModellingHypothesis::AXISYMMETRICALGENERALISEDPLANESTRESS, NumericType, false>& b)
{
os << "εᵗᵒ : " << b.eto << '\n';
os << "σ : " << b.sig << '\n';
os << "εᵉˡ : " << b.eel << '\n';
...
template<typename NumericType>
class VoceBehaviourData<ModellingHypothesis::PLANESTRESS, NumericType, false>
{
...
template<typename NumericType>
std::ostream&
operator <<(std::ostream& os,const VoceBehaviourData<ModellingHypothesis::PLANESTRESS, NumericType, false>& b)
{
os << "εᵗᵒ : " << b.eto << '\n';
os << "σ : " << b.sig << '\n';
os << "εᵉˡ : " << b.eel << '\n';
...
#endif /* LIB_TFELMATERIAL_VOCE_BEHAVIOUR_DATA_HXX */
