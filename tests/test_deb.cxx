...
TEST(t_deb, t_deb)
{
    ...
    auto file_path = "/home/dsiedel/projetcs/lolita/lolita/tests/data/meshes/triangle_structured_00001.msh";
    ...
    auto constexpr t_basis = lolita::Basis::monomial(1);
    ...
    for (auto const & finite_element : {elements->template getElements<2, 0>()[0]})
    {
        ...
        for (auto const & face : finite_element->template getInnerNeighbors<0, 0>())
        {
            ...
            auto constexpr rows_ = lolita::FiniteElementBasisTraits<t_basis>::template getSize<t_face>();
            ...
            for (auto ipf = 0; ipf < lolita::ElementQuadratureRuleTraits<t_face, t_quadrature>::getSize(); ipf++)
            {
                ...
                auto row_cell_vector = finite_element->template getBasisEvaluation<lolita::Basis::monomial(1)>(i_p);
                auto col_face_vector = face->template getBasisEvaluation<lolita::Basis::monomial(1)>(point);
                auto col_cell_vector = finite_element->template getBasisEvaluation<lolita::Basis::monomial(1)>(i_p);
