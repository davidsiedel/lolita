...
main(int argc, char** argv)
{
    ...
    auto newton_step = [&] ()
    {
        ...
        while(iteration < 10)
        {
            system_tick = std::chrono::high_resolution_clock::now();
            // displacement_system->initialize();
            ...
            system_tock = std::chrono::high_resolution_clock::now();
            system_time = std::chrono::duration<double>(system_tock - system_tick).count();
            std::cout << "iteration time : " << std::setprecision(17) << system_time << " s" << std::endl;
            iteration ++;
            total_iteration ++;
        }
