    ...
    struct MeshElement<t_element, t_domain>
    {
        ...
        const
        {
            ...
            for (Integer i = 0; i < ElementTraits<t_element, t_domain>::template getNumInnerNeighbors<t_i, t_j>(); ++i)
            {
                ...
                {
                    auto cpt = MeshElement<t_inner_neighbor, t_domain>();
                    for (Integer j = 0; j < t_inner_neighbor.num_nodes_; ++j)
                    {
                ...
                {
                    inner_neighbor_hash = MeshElement<t_inner_neighbor, t_domain>::getHash(node_tags_[getInnerNeighborNodeConnection<t_i, t_j>(i, 0)]);
                }
    ...
    private:
        ...
        setPhysicalEntities()
        {
            ...
            for (Integer i = 0; i < num_physical_names; ++i)
            {
                ...
                physical_entities_[hash] = std::make_shared<PhysicalEntity>(name, dim);
                offset += 1;
            }
        }
