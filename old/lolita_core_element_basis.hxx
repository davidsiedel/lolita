...
namespace lolita::core::element
{
    ...
    namespace detail
    {
        ...
        template<Element E, BasisName B, Indx K>
        struct FiniteElementBasis;

        template<Element E, Indx K>
        struct FiniteElementBasis<E, BasisName::Monomial, K>
        {
    ...
    struct FiniteElementBasis2
    {
        ...
        struct Implementation : public FiniteElement<E, D, M>
        {
            ...
            {
                auto v = Vector<Real>();
                if (basis_description == Basis(BasisName::Monomial, 1)) {
                    v = detail::FiniteElementBasis<E, BasisName::Monomial, 1>::evaluate(point_arg);
                }
                else if (basis_description == Basis(BasisName::Monomial, 2)) {
                    v = detail::FiniteElementBasis<E, BasisName::Monomial, 2>::evaluate(point_arg);
                }
                else {
                    assert(false);
                }
            ...
            const
            {
                auto v = Vector<Real>();
                if (basis_description == Basis(BasisName::Monomial, 1)) {
                    v = detail::FiniteElementBasis<E, BasisName::Monomial, 1>::evaluate(point_arg, derivative_direction_arg);
                }
                else if (basis_description == Basis(BasisName::Monomial, 2)) {
                    v = detail::FiniteElementBasis<E, BasisName::Monomial, 2>::evaluate(point_arg, derivative_direction_arg);
                }
                else {
                    assert(false);
                }
