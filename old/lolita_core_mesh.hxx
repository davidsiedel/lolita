...
//    using FiniteElementE = core::element::FiniteElementF<E, D, Med>;
    ...
    template<lolita::mesh::Format, lolita::geometry::Domain D, lolita::finite_element::FiniteElementConcept auto... _finite_element>
    struct MeshModule;
    ...
    using DegreeOfFreedomIndex = lolita::core::element::DegreeOfFreedomIndex;
...
//    };
    ...
    template<lolita::mesh::Format Mft, lolita::geometry::Domain _domain, lolita::finite_element::FiniteElementConcept auto... _finite_element>
    struct MeshBase
    {
    ...
    private:
        ...
        using _ElementPtr = std::shared_ptr<FiniteElementFinal<__element, __domain, __finite_element...>>;

        template<Element __element, lolita::geometry::Domain __domain, lolita::finite_element::FiniteElementConcept auto... __finite_element>
        using _ElementPtrMap = std::unordered_map<std::basic_string<lolita::character>, _ElementPtr<__element, __domain, __finite_element...>>;

        using _Elements = lolita::core::geometry::ElementCollection<_ElementPtrMap, _domain, _finite_element...>;

        using _ElementSets = std::unordered_map<std::basic_string_view<lolita::character>, _Elements>;

        using _Module = typename MeshModule<Mft, _domain, _finite_element...>::Module;

        using _Implementation = typename MeshModule<Mft, _domain, _finite_element...>::Implementation;
    ...
    public:
        ...
        _ElementSets element_sets_;

        std::vector<lolita::finite_element::Load> loads_;

//        lolita::finite_element::Loads loads_;

        std::vector<lolita::behaviour::MgisBehaviour> behaviours_;

//        lolita::behaviour::MgisBehaviours behaviours_;
...
//            loadLoad();
        ...
        makeElementSets()
        {
            static_cast<_Implementation *>(this)->makeElementSets();
        }

        void
        makeNodeSets()
        {
            static_cast<_Implementation *>(this)->makeNodeSets();
        }

        void
        makeNodes()
        {
            static_cast<_Implementation *>(this)->makeNodes();
        }
...
//                ptr_node->initialize();
                ptr_node->initialize(unknown_indices, loads_, behaviours_);
                nodes.insert({hash, ptr_node});
            };
            /*
             *
             */
            auto ptr_node = createElement<element::pnt_00>();
            initialize_element(ptr_node);
        }

        void
        loadLoad()
        {
            for (auto & [set_name, element_tags]: element_sets_) {
                for (auto & load : loads_) {
                    if (load.domain_tag_ == set_name) {
                        auto set_load = [&] <lolita::index _i = 0u, lolita::index _j = 0u, lolita::index _k = 0u> (
                                auto &
                                self
                        )
                        constexpr mutable
                        {
                            for (auto & [elem_tag, elem] : element_tags.template getElements<_i, _j>()) {
                                auto unknown_tag = elem->template getElement<_k>()->getUnknownTag();
                                if (unknown_tag == load.unknown_tag_) {
                                    elem->template getElement<_k>()->degrees_of_freedom_[load.components_.row_][load.components_.col_].load_ = load.load_;
                                    elem->template getElement<_k>()->degrees_of_freedom_[load.components_.row_][load.components_.col_].binding_index_ = this->unknown_indices[_k].binding_index;
                                    this->unknown_indices[_k].binding_index += elem->template getElement<_k>()->degrees_of_freedom_[load.components_.row_][load.components_.col_].unknown_coefficients_.size();
                                }
                            }
                            if constexpr (_k < sizeof...(_finite_element) - 1) {
                                self.template operator()<_i, _j, _k + 1u>(self);
                            }
                            else if constexpr (_j < lolita::core::element::numElements<_domain, _i>() - 1) {
                                self.template operator()<_i, _j + 1u, 0u>(self);
                            }
                            else if constexpr (_i < lolita::core::element::numElements<_domain>() - 1) {
                                self.template operator()<_i + 1u, 0u, 0u>(self);
                            }
                        };
                    }
                }
            }
            std::cout << this->unknown_indices[0].binding_index << std::endl;
            std::cout << this->unknown_indices[0].binding_index << std::endl;
        }
        ...
        void
        makeCell(
                std::array<lolita::index, _element.num_nodes_> const &
                cell_node_tags
        )
        {
            /*
            ...
            )
            {
                if constexpr (!element::PointConcept<__element>) {
                    std::vector<std::basic_string_view<lolita::character>> domain_names;
...
//                    auto const & node_hashes = std::get<0>(std::get<0>(element_tags));
             ...
             *
             */
            auto build_element = [&] <Element __element> (
                    std::array<lolita::index, __element.num_nodes_> const &
                    element_node_tags,
            ...
            )
            {
                ...
                auto & elements = elements_.template getElements<_element_coordinates[0], _element_coordinates[1]>();
...
//                ptr_element->domains_ = element_domains;
//                ptr_element->initialize();
                ptr_element->initialize(unknown_indices, loads_, behaviours_);
                for (auto const & domain : element_domains) {
                    element_sets_[domain].template getElements<_element_coordinates[0], _element_coordinates[1]>().insert({element_hash, ptr_element});
                }
             ...
             *
             */
            auto initialize_element = [&] <Element __element, lolita::index _i = 0, lolita::index _j = 0> (
                    std::array<lolita::index, __element.num_nodes_> const &
                    element_node_tags,
                    std::shared_ptr<FiniteElementFinal<__element, _domain, _finite_element...>> &
                    ptr_element,
                ...
                if constexpr (!element::PointConcept<__element>) {
                    ...
                    for (auto i = 0; i < element_component_array.size(); ++i) {
                        ...
                        if (!components.contains(component_hash)) {
                            auto ptr_component = createElement<_component>();
                    ...
                    if constexpr (_is_initialized) {
                        build_element.template operator()<__element>(element_node_tags, ptr_element);
                    }
                }
            };
            /*
             *
             */
            auto ptr_cell = createElement<_element>();
            initialize_element.template operator ()<_element>(cell_node_tags, ptr_cell, initialize_element);
        }
        ...
        )
        {
            ...
            mutable
            {
                static_cast<_Implementation *>(this)->template makeCells<element::element<_domain, _domain.dim_, I>()>();
        ...
        makeNeighbourhood()
        {
             ...
             *
             */
            auto make_element_neighbourhood = [&] <Element _element, lolita::index _k = 0u> (auto & self)
            mutable
            {
                if constexpr (!element::PointConcept<_element>) {
                    auto const constexpr poss = element::elementPosition<_domain, _element>();
                    ...
                    if constexpr (_k < element::numElements<_domain, poss[0]>() - 1) {
                        self.template operator()<_element, _k + 1>(self);
                    }
                }
            };
            /*
             *
             */
            auto make_neighbourhood = [&] <lolita::index _i = 1u, lolita::index _j = 0u> (
                    auto & self
            )
            mutable
            {
                make_element_neighbourhood.template operator()<element::element<_domain, _i, _j>()>(make_element_neighbourhood);
                if constexpr (_j < element::numElements<_domain, _i>() - 1) {
                    self.template operator()<_i, _j + 1u>(self);
                }
                else if constexpr (_i < element::numElements<_domain>() - 1) {
        ...
        )
        {
            ...
            mutable
            {
                ...
                else if constexpr (I < element::numElements<_domain>() - 1) {
    ...
    struct MeshModule<lolita::mesh::Format::Gmsh, D, _finite_element...>
    {
...
//                                    tags = get_subs(j, k + 1, tags, get_subs);
        ...
        struct Implementation : public MeshBase<lolita::mesh::Format::Gmsh, D, _finite_element...>
        {
...
//                                        line_stream = std::basic_string<lolita::character>Stream(this->module_.file_.get(line_start + offset));
                                        ...
                                        /*
                                         * Appending the element set name to the node
                                         */
                                        nodes[node_hash]->domains_.push_back(node_set_name);
                                        /*
                                         *
                                         */
                                        offset += 1;
