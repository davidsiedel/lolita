#ifndef F8836C43_195D_4FBE_ADF4_49F686097E23
#define F8836C43_195D_4FBE_ADF4_49F686097E23
...
namespace lolita
{
    ...
    namespace tensor
    {
        ...
        template<typename T_, typename Scalar_, lolita::Integer rank_>
        concept TensorConcept = requires
        {
            ...
            //  * 
            //  */
            // requires std::convertible_to<std::decay_t<T_>, Eigen::Tensor<Scalar_, rank_>>;

            /**
             * @brief 
             * 
             */
            requires std::derived_from<std::decay_t<T_>, Eigen::TensorBase<std::decay_t<T_>>>;

            /**
             * @brief 
             * 
             */
            requires std::is_same_v<Scalar_, typename std::decay_t<T_>::Scalar>;

            /**
             * @brief 
             * 
             */
            requires rank_ == std::decay_t<T_>::NumDimensions;
            
        };

        template<typename Scalar_, lolita::Integer... t_>
        using StaticTensor = Eigen::TensorFixedSize<Scalar_, Eigen::Sizes<t_...>>;

        template<typename Scalar_, lolita::Integer t_>
        using DynamicTensor = Eigen::Tensor<Scalar_, t_>;

        template<typename Tensor_>
        using TensorSpan = Eigen::TensorMap<Tensor_>;
...
#endif /* F8836C43_195D_4FBE_ADF4_49F686097E23 */
